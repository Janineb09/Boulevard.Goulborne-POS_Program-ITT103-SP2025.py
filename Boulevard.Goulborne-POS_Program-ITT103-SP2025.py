...
def remove_from_cart():
    while True:
        ...
        more_removal = input("Do you want to remove another item? (yes/no): ").strip().lower()
        if more_removal != "yes":
            break
...
def view_cart():
    print("\nYour Cart:")
    total = sum(products[item][0] * qty for item, qty in cart.items())
    for item, qty in cart.items():
        print(f"{item}: {qty} x ${products[item][0]} = ${products[item][0] * qty}")
